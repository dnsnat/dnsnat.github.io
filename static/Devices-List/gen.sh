...
[ -z "`awk --version | grep 'GNU'`" ] && IS GNU=1
#
...
do
	...
	# $('#ov2')
	# $('#ov4')
	out=$out`top -bn1 | awk '/MiB Mem/ {printf("\"Mem\":[%s, %s, %s, %s],", $4, $6, $8, $10)} /MiB Swap/ {printf("\"Swap\":[%s, %s, %s, %s]", $3, $5, $7, $9)}'`
	out=$out","
	...
	then
		...
		timer=0
		if [[ ! -z $YUM_CMD ]]; then
			softlist=`yum check-update | grep 'updates' | awk 'BEGIN {printf("[")}; {printf("[\"%s\",\"%s\",\"%s\"]",$1,$2,$3)}; END {printf("]")}' | sed "s/\]\[/\],\[/g"`
			ins_type=1
		elif [[ ! -z $APT_GET_CMD ]]; then
			softlist=`apt-get --just-print upgrade 2>&1 | grep Inst | sed 's/[][()]//g' | awk 'BEGIN {printf ("[")} {printf("[\"%s\", \"%s\", \"%s\", \"%s\", \"%s\"],", $2, $3, $4, $5, $6)} END {printf("]")}' | sed 's/,]/]/'`
			ins_type=2
		elif [[ ! -z $BREW_CMD ]]; then
			softlist=`brew outdated --verbose | sed 's/[()<]//g' | sed 's/  / /g' | awk 'BEGIN {printf("[")}; {printf("[\"%s\",\"%s\",\"%s\"]",$1,$2,$3)}; END {printf("]")}' | sed "s/\]\[/\],\[/g"`
			ins_type=3
		else
			softlist="[]"
			ins_type=0
	...
	out=$out"\"Install_System_Type\":"$ins_type
	out=$out","
	out=$out"\"Update\":"$softlist
