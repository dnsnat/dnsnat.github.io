...
requestNewData = function(keyvalue){
	...
	if (!onrequestIn && cacheSendKeys){
		onrequestIn=true
		$.post(oterm_server + 'in',{type:cacheSendKeys}, function(){ onrequestIn=false; requestNewData(); })
		cacheSendKeys=''
	}
	if (!onrequestOut){
		onrequestOut=true
		$.get(oterm_server + 'out',{pos:readDataPos},updateRequestData,'text')
	}
}
...
updateGeometry = function(){
	...
	if (r!=nrRows || c!=nrCols){
		...
		$.post(oterm_server + 'resize',{width:nrRows, height:nrCols})
	}
}
...
$(document).ready(function(){
	...
	onrequestOut=true
	$.get(oterm_server + 'out?initial',updateRequestData, 'text')
